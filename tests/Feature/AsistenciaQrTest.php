...
class AsistenciaQrTest extends TestCase
{
    ...
    public function test_un_docente_registra_asistencia_escaneando_qr(): void
    {
        ...
        $qr = QrToken::factory()->create(['school_id' => $school->id, 'alumno_id' => $alumno->id]);

        $docente = User::factory()->create(['school_id' => $school->id, 'email_verified_at' => now()]);
        $docente->assignRole('docente');

        $contenido = app(QrTokenService::class)->contenido($qr);
    ...
    public function test_un_qr_de_otra_escuela_es_rechazado(): void
    {
        ...
        // Token del alumno de la escuela B.
        $qrB = QrToken::factory()->create(['school_id' => $escuelaB->id, 'alumno_id' => $alumnoB->id]);
        $contenido = app(QrTokenService::class)->contenido($qrB);
